        ...
        <script type="text/javascript">
			...
			/* function Handle(node) {
	        	...
	        	for (var i = 0; i < node.length; i++) {
	        		html+='<li id="'+node[i].id+'" class="">';
	        		html+='<h3><a href="" title="'+node[i].name+'">'+node[i].name+'</a></h3>';
					 if(node[i].child.length!=0){
						html+='<div id="cat_1_menu" class="cat_menu clearfix" style="">';
								  for (var i = 0; i < node[i].child.length; i++) {
									html+='<dl class="clearfix">';
										html+='<dt><a href="'+node[i].child[i].name+'" href="">'+node[i].child[i].name+'</a></dt>';
											html+='<dd>';
											if(node[i].child[i].child.length!=0){
												for (var int = 0; int < node[i].child[i].child.length; int++) {
													html+='<a href="">'+node[i].child[i].child[i].name+'</a>';
												}
											} 
        	...
        	var dom = document.getElementsByClassName("shop_hd_menu_all_category_hd_menu")[0];
        	dom.innerHTML=html; 
        	debugger;
			} */
        </script>
			...
			<div class="shop_hd_menu_all_category shop_hd_menu_hover"><!-- 首页去掉 id="shop_hd_menu_all_category" 加上clsss shop_hd_menu_hover -->
				...
				<div id="shop_hd_menu_all_category_hd" class="shop_hd_menu_all_category_hd">
					<ul class="shop_hd_menu_all_category_hd_menu clearfix">
						...
						<li id="cat_${category.id}" class="">
							<h3><a href="" title="${category.name}">${category.name}</a></h3>
							<c:if test="${!empty category.child}">
								<div id="cat_1_menu" class="cat_menu clearfix" style="">
									<c:forEach items="${category.child}" var="category_son">
										<dl class="clearfix">
											<dt><a href="${category_son.name}" href="">${category_son.name}</a></dt>
											<dd>
												<c:if test="${!empty category_son.child}">
													<c:forEach items="${category_son.child}" var="category_grand">
														<a href="">${category_grand.name}</a>
													</c:forEach>
												</c:if>
